/**
 * TERAGON AI BUSINESS OS — Layout barrel (app shell components).
 */
export { RightPrimaryNavigation } from "./RightPrimaryNavigation";
export type {
  RightPrimaryNavigationProps,
  NavItem,
  NavGroupSpec,
  RenderNavLink,
} from "./RightPrimaryNavigation";

export { CompactTopHeader } from "./CompactTopHeader";
export type { CompactTopHeaderProps, ShellUser } from "./CompactTopHeader";

export { LeftIntelligenceRail } from "./LeftIntelligenceRail";
export type { LeftIntelligenceRailProps } from "./LeftIntelligenceRail";

export { MainOperationalWorkspace } from "./MainOperationalWorkspace";
export type { MainOperationalWorkspaceProps } from "./MainOperationalWorkspace";

export { AppShell } from "./AppShell";
export type { AppShellProps } from "./AppShell";
