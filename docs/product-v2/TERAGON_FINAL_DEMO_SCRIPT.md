...
| 9 | **`workflow_failed` → Governed Follow-up Task** | `/ai-workspace` then `/` | **Produce the canonical failure via product controls (no console):** with Obsidian *not* connected, on the governed pack click **"התחל תהליך"** — the workflow runs (Orchestrator → Wiki → Vault read) and **fails** (`WORKFLOW_FAILED`, Obsidian unavailable). Return to `/`: the Action-Inbox shows **`workflow_failed`** → click **"צור משימת מעקב"** | Command Center "1 נכשל"; proposal modal: exact Task preview, **CREATE ONE TASK**, source signal, owner = trusted actor; **no Task yet** | Any actionable signal (e.g. a `proposal_pending` from running step 4 with Obsidian connected) also exposes the CTA |
